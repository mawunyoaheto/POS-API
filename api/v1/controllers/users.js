...
async function getUserCategories(req, res, error) {
  ...
  try {
    ...
    pool.query('select * from user_categories', function (err, recordset) {
      ...
      if (recordset.rowCount > 0) {
        // send records as a response
       usersResp = respBody.ResponseBody('success', recordset.rows, recordset.rowCount + ' record(s) found');
        resp.json(201,usersResp);

      } else {
         usersResp = respBody.ResponseBody('success', '', recordset.rowCount + ' record(s) found');
          resp.json(402,usersResp);
      }
    });

  } catch (error) {
     usersResp = respBody.ResponseBody('failed', '', 'failed with error: ' + helper.parseError(error));
      resp.json(500,usersResp);
  }
